jQuery(document).ready(function ($) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function showTab(n) {
      console.log("showTab chamada com n =", n, "e x.length =", x.length);

      var x = document.getElementsByClassName("tab");

      // Se não houver tabs ou n for um índice inválido, retorne.
      if (!x.length || n < 0 || n >= x.length) return;

      x[n].style.display = "block";

      // ... e fix the Previous/Next buttons:
      if (n == 0) {
    ...
    function nextPrev(n) {
      var x = document.getElementsByClassName("tab");
      console.log("nextPrev chamada com n =", n, "e currentTab =", currentTab);

      // Se não houver tabs, retorne.
      ...
      if (currentTab >= 0 && currentTab < x.length) {
        // Verifica se currentTab é um índice válido
    ...
    if (!x || !x.length || typeof x[n] === "undefined") {
      console.error(
        "Erro: showTab chamada com n inválido ou x não está definido corretamente. n =",
        n,
        "x =",
        x
      );
    ...
    if ($(x[currentTab]).find('input[type="radio"]').length > 0) {
      ...
      $.each(radionames, function (index, value) {
        ...
        if (radios.length == 0) {
          valid = false;
          message +=
            $("input[name=" + value + "]:first").attr("data-error-message") +
            "\n";
        }
      });
    }
  ...
  function envioRD(tab) {
    ...
    if (
      jQuery("#email").val() &&
      regex.test(jQuery("#email").val()) &&
      nomeSobrenome.test(campo)
    ) {
      var data_array = [
